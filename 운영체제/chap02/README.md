...
- 시스템 콜이란 사용자 프로그램이 운영체제의 서비스를 받기 위해 커널 함수를 호출한 것이다.
...
- Device Controller가 IO장치를 제어하며, 작업 공간으로 로컬 버퍼를 사용하여 데이터를 로컬 버퍼에 저장한다.

## CPU

- CPU는 매 순간 메모리의 특정 위치의 기계어(인스트럭션)을 읽어와서 실행한다.
- CPU의 레지스터 중 하나가 메모리의 주소를 가지는데 그 주소를 가리키는 메모리 위치에서 인스트럭션을 읽어와서 실행한다.
...
- 인터럽트는 타이머 입출력 장치 등이 발생시킨 하드웨어 인터럽트를 주로 의미 하지만 Exception이나 System call과 같은 Trap도 인터럽트를 의미하기도 한다.
- 운영체제는 각 인터럽트 마다 수행되어야 할 처리 루틴을 가지고, 처리 루틴의 주소를 가진 것이 인터럽트 벡터이다.
- 인터럽트
...
- 구현 방법으로 IO가 완료될 때까지 CPU를 낭비시키는 방법과 IO가 완료될 때까지 제어권을 뺏어와서 다른 프로그램에게 주는 방법이 있다.
...
![Sync_Async.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f2492d42-eaf1-4b96-8a22-ddd7ddda2512/Sync_Async.png)
...
- 작은 단위가 아닌 블록 단위로 인터럽트를 발생 시켜서 인터럽트의 빈도를 줄인다.
